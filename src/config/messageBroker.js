...
class MessageBroker {
  ...
  async init() {
    this.connection = await amqp.connect(
      `amqp://${process.env.MESSAGE_BROKER_HOST}` || 'amqp://localhost'
    )
  ...
  /**
   * @param {String} queue Queue name
   * @param {Function} handler Handler that
   * will be invoked with given message
   * and acknowledge function (msg, ack)
   */
